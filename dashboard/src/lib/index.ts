...
export async function createDevice(deviceName?: string): Promise<void> {
	try {
		...
		const channel = await createChannel('channel-' + deviceName, '', [
			{ type: 'cxc', source: 'cxc' }
		]);
		// Create a device
		if (channel) {
			...
			const deviceResponse = await fetch(
				`${SLU_API_BASE_URL}/one-shot-device/create/${channel?.channelAddress}/${
					get(authenticationData)?.did
				}`,
				{
...
export async function isAuthenticDevice(deviceId: string): Promise<boolean> {
	try {
		...
		const isAuthenticResponse = await fetch(
			`${SLU_API_BASE_URL}/authenticity/prove?did=${deviceId}&from=${registrationDate}&to=${today}`,
			{
				headers: {
					'X-API-KEY': import.meta.env.VITE_SLU_STATUS_API_KEY
				}
