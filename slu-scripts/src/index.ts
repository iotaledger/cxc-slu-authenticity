...
const argv = yargs
	.command('encrypt', 'Encrypting data', (yargs) =>
		yargs
			.option('key_file', { desc: 'The location of the key file.' })
			.option('input', { desc: 'The location of the data to encrypt.' })
			.option('dest', { desc: 'The destination where the encrypted data has to be stored.' })
	)
