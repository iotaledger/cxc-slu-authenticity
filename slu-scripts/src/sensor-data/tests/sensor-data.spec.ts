...
describe('Send sensor data tests', () => {
	...
	it('should fail to send data: no valid api key provided', async () => {
		try {
			await sendData(encryptedDataPath, keyFilePath, isConfigPath, collectorBaseUrl, {}, isAuthUrl, apiKey, jwt);
		} catch (ex: any) {
	...
	it('should fail to send data: env variable not provided', async () => {
		try {
			...
			expect(ex.message).toBe('One or all of the env variables are not provided: --input_enc, --key_file, --config, --collector_data_url, --collector_url, --is_url, --api_key, --jwt');
		}
	});
	...
	it('should send data with retry: jwt expired', async () => {
		 ...
		 * **/
		const resp = {
			data: { jwt: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..UZGr5c2dZvQmugahPKOcNhqeE92CeQZhaEuZ3YRmwJ4" },
			headers: {},
			config: {},
			status: 401,
