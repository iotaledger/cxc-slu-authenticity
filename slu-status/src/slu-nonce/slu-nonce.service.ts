...
export class SluNonceService {
	...
	async saveSluNonce(sluNonce: SluNonceDto): Promise<SluNonce> {
		return (await new this.sluNonce(sluNonce).save()).toObject();
	}

	async getSluNonce(sluId: string, creator: string): Promise<SluNonce> {
		const sluNonce: SluNonce = await this.sluNonce.findOne({ sluId }, undefined, { fields: { _id: 0 } }).lean();
	...
	async getSluNonces(sluNonces: string[], creator: string): Promise<SluNonce[]> {
		const sluNoncesDb: SluNonce[] = await this.sluNonce.find().where('sluId').in(sluNonces).lean();
